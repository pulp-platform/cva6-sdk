...
int probe_node(struct platform_device *pdev,
    ...
    pr_info("Looking around %s", pdev->dev.of_node->name);
    for_each_child_of_node(pdev->dev.of_node, tmp_np)
    {
	pr_info("tmp_np %s", tmp_np->name);
    }
	...
	pr_info("tmp_res : %p", tmp_res.start);
