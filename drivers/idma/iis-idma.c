...
static void iis_idma_desc_free(struct virt_dma_desc *vd)
{
	...
	for (i = sw_desc->n_hw_descs - 1; i >= 0; --i) {
		dma_pool_free(sw_desc->hw_desc_allocator, &sw_desc->hw_descs[i], i == 0 ?
				sw_desc->first : sw_desc->hw_descs[i - 1]->next);
	}
...
static int iis_idma_probe(struct platform_device *pdev)
{
	...
	ret = of_dma_controller_register(of_node, of_dma_simple_xlate, idma_dev);
	if (ret)
